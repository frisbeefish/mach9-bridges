...
import { STATE_ABBREV_TO_FULL_NAME } from "../../shared/constants";
...
export default function CountryMap() {
  ...
  const selectedStatePath = useRef(null);
  ...
  //
  // Pre-select PA at startup. This emulates the user clicking on Pennsylvania on the main USA map.
  //
  useEffect(() => {
    const pa = document.querySelector("path#PA");
    selectState({
      target: pa,
    });
  }, []);
  ...
  /**
   * The user clicked on an SVG path for a state within the USA.
   */
  function selectState(event) {
    //
    // De-select any previously selected state.
    //
    if (selectedStatePath.current) {
      selectedStatePath.current.classList.remove("selected");
    }

    //
    // Show the state the user clicked on as selected.
    //
    event.target.classList.add("selected");
    selectedStatePath.current = event.target;

    //
    // This updates the global/reactive data representing which state is currently selected.
    // When this is set, it causes other parts of the app - such as the main MAP with bridges for the
    // selected state - to update.
    //
    setSelectedState(event.target.id);
  }

  /**
   * Update the UI as the user hovers over and out of states on the map of the USA.
   */
  function hoverOverState(event) {
    setHoveredState(event.target.id);
  }
  function hoverOutState(event) {
    setHoveredState(null);
  }

  return (
    <Box className="country-map" display="flex" flexDirection="column">
      <Heading as="h2" mb="20px">
        State:{" "}
        {hoveredState
          ? STATE_ABBREV_TO_FULL_NAME[hoveredState]
          : STATE_ABBREV_TO_FULL_NAME[selectedState]}
      </Heading>
      ...
      >
        <g id="g5">
          <path
            id="HI"
            onClick={selectState}
            onMouseEnter={hoverOverState}
            onMouseLeave={hoverOutState}
